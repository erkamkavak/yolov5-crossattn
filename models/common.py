...
class AttentionV2(nn.Module): 
    def __init__(self, q_dim, k_dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.wq = nn.Linear(q_dim, self.dim, bias=qkv_bias)
        self.wk = nn.Linear(k_dim, self.dim, bias=qkv_bias)
        self.wv = nn.Linear(k_dim, self.dim, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(self.dim, q_dim)
        self.proj_drop = nn.Dropout(proj_drop)
...
class CrossAttentionV3(nn.Module): 
    def __init__(self, dim, q_size_ratio, k_size_ratio, num_heads=8, qkv_bias=False, qk_scale=None, 
        ...
        q_dim = self.dim
        k_dim = q_dim * k_size_ratio // q_size_ratio
        
        min_size = 10
        q_resized_size = (min_size * 32) // q_size_ratio
        k_resized_size = (min_size * 32) // k_size_ratio

        self.q_downsample = nn.AdaptiveAvgPool2d((q_resized_size, q_resized_size))
        self.k_downsample = nn.AdaptiveAvgPool2d((k_resized_size, k_resized_size))

        self.attention = AttentionV2(q_dim, k_dim, num_heads, qkv_bias, qk_scale, attn_drop, proj_drop)
    ...
    def forward(self, x): 
        ...
        u2 = self.q_downsample(u2) # u2 -> (B, C, S, S)
        resized_img_size = u2.shape[2:4]
        u1 = u1.permute(0, 2, 3, 1).contiguous().view(B, -1, C1)
        u2 = u2.permute(0, 2, 3, 1).contiguous().view(B, -1, C2)
        # u1=u2 -> (B, S*S, C)

        y = self.attention(q=u2, k=u1, v=u1) # y -> (B, S*S, C)
        y = y.transpose(1, 2).contiguous().view(B, C2, *resized_img_size) # y -> (B, C, S, S)

        y = nn.Upsample(size=(H2, W2), mode="bilinear")(y)
        y += initial_input

        return y
    

class CrossAttentionPosEmbed(nn.Module): 
    def __init__(self, dim, q_size_ratio, k_size_ratio, num_heads=8, qkv_bias=False, qk_scale=None, 
                 attn_drop=0., proj_drop=0., resid_pdrop=0.1):
        super().__init__()
        self.dim = dim
        ...
        self.pos_emb_q = nn.Parameter(torch.zeros(1, q_resized_size * q_resized_size, self.dim))
        self.pos_emb_k = nn.Parameter(torch.zeros(1, k_resized_size * k_resized_size, self.dim))
...
class SelfAttentionPosEmbed(nn.Module): 
    def __init__(self, dim, q_size_ratio, k_size_ratio, num_heads=8, qkv_bias=False, qk_scale=None, 
                 attn_drop=0., proj_drop=0., resid_pdrop=0.1):
        super().__init__()
        self.dim = dim
        ...
        # positional embedding parameter (learnable)
        self.pos_emb = nn.Parameter(torch.zeros(1, q_resized_size * q_resized_size + k_resized_size * k_resized_size, self.dim))
    ...
    def forward(self, x): 
        ...
        attn_input = torch.cat([u1, u2], dim=1)
        attn_input = self.drop(self.pos_emb + attn_input) # (B, S1*S1 + S2*S2, C)

        y = self.attention(q=attn_input, k=attn_input, v=attn_input) # y -> (B, S1*S1 + S2*S2, C)
        y = y[:, u1.shape[1]:, :].transpose(1, 2).contiguous().view(B, C2, *resized_img_size)
...
# Swin transformer implementation(taken from https://github.com/microsoft/Swin-Transformer/blob/main/models/swin_transformer.py)
...
class SwinTransformer(nn.Module):
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            ...
            # only get keys of pretrained_dict starts with backbone
            pretrained_dict = {k[len('backbone.'):]: v for k, v in pretrained_dict.items() if k.startswith('backbone.')}
...
swin_transformer = None
class SwinTransformerYoloLayer(nn.Module): 
    def __init__(self, layer_index, embed_dim=64, img_size=224, patch_size=4): 
        super().__init__()
        self.layer_index = layer_index

        global swin_transformer
        if swin_transformer == None: 
            swin_transformer = SwinTransformer(embed_dim=embed_dim, pretrain_img_size=img_size, patch_size=patch_size)
            swin_transformer.init_weights('../mask_rcnn_swin_tiny_patch4_window7_1x.pth')
            swin_transformer.frozen_stages = 4
            swin_transformer._freeze_stages()

            self.patch_embed = swin_transformer.patch_embed
            self.ape = swin_transformer.ape
            if self.ape: 
                self.absolute_pos_embed = swin_transformer.absolute_pos_embed
            self.pos_drop = swin_transformer.pos_drop

        self.layer = swin_transformer.layers[self.layer_index]
        self.layer_norm = getattr(swin_transformer, f'norm{self.layer_index}')

    def forward(self, x):
        # print(f'layer {self.layer_index} x.shape: {x.shape}')

        global swin_transformer
        if self.layer_index == 0: 
            x = self.patch_embed(x) # x -> (B, C, Ph, Pw)
            if self.ape:
                x = x + self.absolute_pos_embed

        B, C, Wh, Ww = x.shape # x -> (B, C, Ph, Pw)
        x = x.flatten(2).transpose(1, 2) # x -> (B, Ph*Pw, C)            
        if self.layer_index == 0:
            x = self.pos_drop(x)  # x -> (B, Ph*Pw, C)
        else: 
            x = swin_transformer.layers[self.layer_index - 1].downsample(x, Wh, Ww) # x -> (B, C, Ph, Pw)
            Wh, Ww = (Wh + 1) // 2, (Ww + 1) // 2
        
        x_out, H, W, x, Wh, Ww = self.layer(x, Wh, Ww) # x -> (B, Ph*Pw, C)
        x_out = self.layer_norm(x_out) # x -> (B, Ph*Pw, C)
        # x = x.transpose(1, 2).reshape(B, -1, Wh, Ww) # x -> (B, C, Ph, Pw)
        x_out = x_out.transpose(1, 2).reshape(B, -1, H, W) # x -> (B, C, Ph, Pw)

        # print(f'layer {self.layer_index} x.shape: {x_out.shape}')
        return x_out
